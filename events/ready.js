...
module.exports = (client) => {
	...
	function setActivity() {
		const status = [`in ${client.guilds.size} servers`, `with ${client.users.size} users`, "m!help for help", "with Kiana", "with Bronya's drills", "with a cat"];
		const info = status[Math.floor(Math.random() * status.length)];
